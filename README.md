...
- Write a class method, `.all`, that returns the `@@all` variable. From inside
  ...
  might be interacting with our `Dog` class from the outside, this `.all` class
  method acts as our direct interface to the `@@all` variable.

- Write a class method, `.print_all`, that iterates over all of the individual
  dogs stored in the `@@all` array and `puts` out their name to the terminal.
